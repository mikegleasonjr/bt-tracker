...
Http.prototype.canCompressResponse = function(req, res) {
    var type = res.getHeader('Content-Type');
    return this.config.compress === true && type && compressible(type);
};
...
Http.prototype.onAnnounce = function(req, res) {
    extend(req.query, { ip: req.query.ip || req.ip });
    ...
    this.engine.announce(req.query, function(err, swarm) {
        ...
        var peers = Number(req.query.compact) === 1 ?
            ...
            this._buildPeersResponse(swarm.peers, Number(req.query.no_peer_id) === 1);
...
Http.prototype._buildPeersResponse = function(peers, noPeerId) {
