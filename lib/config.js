...
Config.prototype.parse = function(cla, noCkecks) {
    return (Array.isArray(cla) ? yargs(cla) : yargs)
        ...
        .describe('mongodb-host', 'MongoDB server host (when --backend mongodb)')
        .requiresArg('mongodb-host')
        .default('mongodb-host', process.env.BTT_MONGODB_HOST || 'localhost')
        .describe('mongodb-port', 'MongoDB server port (when --backend mongodb)')
        .requiresArg('mongodb-port')
        .default('mongodb-port', Number(process.env.BTT_MONGODB_PORT) || 27017)
        ...
        .check(function(argv) {
            ...
            for (var a in { 'http-port': '', 'udp-port': '', 'interval': '', 'max-peers': '', 'mongodb-port': '' }) {
