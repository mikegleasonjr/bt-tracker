...
Engine.prototype.connect = function(callback) {
    var randConnId = crypto.pseudoRandomBytes(8);

    this.backend.addConnId(randConnId.toString('hex'), function(err) {
        callback(err, randConnId);
    });
};

Engine.prototype.isConnected = function(connId, callback) {
    this.backend.isConnId(connId.toString('hex'), callback);
};
