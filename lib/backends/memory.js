...
MemoryBackend.prototype.deletePeer = function(infoHash, peerId, callback) {
    var swarm = this.swarms[infoHash];
    if (swarm) {
...
MemoryBackend.prototype.incrementSeeders = function(infoHash, callback) {
    this._getSwarm(infoHash).seeders++;
    callback(null);
};

MemoryBackend.prototype.decrementSeeders = function(infoHash, callback) {
...
MemoryBackend.prototype.incrementLeechers = function(infoHash, callback) {
    this._getSwarm(infoHash).leechers++;
    callback(null);
};

MemoryBackend.prototype.decrementLeechers = function(infoHash, callback) {
    var swarm = this._getSwarm(infoHash);
    if (swarm.leechers > 0) {
        swarm.leechers--;
    }
