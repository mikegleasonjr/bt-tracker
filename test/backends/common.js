...
module.exports = function(backend, name) {
    var givenInfoHash = crypto.pseudoRandomBytes.bind(null, 20);
    var givenPeerId = givenInfoHash;

    describe('when getting the backend name', function() {
        it('should be ' + name, function() {
            backend.getName().should.equal(name);
        });
    });

    /*
    describe('when getting an unknown swarm', function() {
        it('should return an empty swarm', function(done) {
            var infoHash = givenInfoHash();

            backend.getSwarm(infoHash, fixtures.getSwarmOptions, function(err, swarm) {
                should(err).be.empty;
                swarm.should.eql({
                    leechers: 0,
                    seeders: 0,
    ...
    describe('when setting a peer', function() {
        it('should be added to the swarm', function(done) {
            var infoHash = givenInfoHash();
            var peerId = givenPeerId();

            var setPeer = function(callback) {
                backend.setPeer(infoHash, peerId, fixtures.setPeerPeer, callback);
            };

            var assertPeerInSwarm = function(callback) {
                backend.getSwarm(infoHash, fixtures.getSwarmOptions, function(err, swarm) {
                    swarm.peers.should.containEql({
                        id: peerId,
                        ip: '7.5.6.3',
                        port: 2874
                    });
            ...
            async.series([setPeer, assertPeerInSwarm], done);
        });
        ...
        it('should overwrite an existing one', function(done) {
            ...
            var setPeer = function(callback) {
                backend.setPeer(infoHash, peerId, peerWith({ ip: '1.2.3.4', port: 1234 }), callback);
            };

            var overwritePeer = function(callback) {
                backend.setPeer(infoHash, peerId, peerWith({ ip: '5.6.7.8', port: 5678 }), callback);
            };

            var assertPeerOverwritten = function(callback) {
                backend.getSwarm(infoHash, fixtures.getSwarmOptions, function(err, swarm) {
                    swarm.peers.length.should.be.exactly(1);
                    swarm.peers.should.containEql({
                        id: peerId,
                        ip: '5.6.7.8',
                        port: 5678
            ...
            async.series([setPeer, overwritePeer, assertPeerOverwritten], done);
        });
    });
    */
